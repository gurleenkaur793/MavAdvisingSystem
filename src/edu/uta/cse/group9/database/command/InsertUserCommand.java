...
public class InsertUserCommand extends SQLCommand {
	...
	void queryDB() throws Exception {
		String query = String.format(
				...
				+ "password_expiration, firstname, lastname, uta_id, user_type) "
				+ "VALUES (?,?,?,?,?,?,?,?,?)",
		...
		try {
			...
			System.out.println("Assigned id " + this.user.getId() + " for class " + this.user.getClass().getName());
			System.out.println("Attempt to invoke... " + methodName);
	...
	private void insertStudentExtra(Student user) throws Exception {
		System.out.println("Adding extra for Student!");
		String query = String.format(
	...
	private void insertAdvisorExtra(Advisor user) throws Exception {
		System.out.println("Adding extra for Advisor!");
		String query = String.format(
