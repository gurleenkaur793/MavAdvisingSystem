...
import edu.uta.cse.group9.database.command.AssignStudentCommand;
...
import edu.uta.cse.group9.database.command.GetAllStudentCommand;
...
public class MySQLDatabaseManager implements DatabaseManagerImpl {
	...
	public List<Student> getAllStudentList() throws Exception{
		SQLCommand command = new GetAllStudentCommand();
		command.execute();
		return (List<Student>) command.getResult();
	}

	@Override
	public boolean assignStudent(int advisorId, int noOfStudents, String startElem, String endElem)	throws Exception {
		SQLCommand command = new AssignStudentCommand(advisorId, noOfStudents, startElem, endElem);
		command.execute();
		return (Boolean) command.getResult();
	}
